...
local opt = lapp[[
	 ...
	 -z,--momentum      (default 0)           momentum, for SGD only
...
--      model:add(nn.ReLU())
      ...
      ------------------------------------------------------------
	 model:add(nn.Reshape(train.data:size(2)))
	 model:add(nn.Linear(train.data:size(2),#classes))
      ------------------------------------------------------------
...
--      local inputs = torch.Tensor(opt.batchSize,1,geometry[1],geometry[2])
 ...
 -- just in case:
	 ...
	 --penalties (Cross class acc diff):
	 local num_class_z = confusion.mat[1][1] + confusion.mat[1][2]
	 ...
	 if num_class_z ~= 0 and num_class_o ~= 0 then
	    ...
	    local acc_c2 = confusion.mat[2][2] / ( confusion.mat[2][1] + confusion.mat[2][2] ) 
	    local diff_acc = acc_c1 - acc_c2
--	    print(f)
--	    print(math.abs(diff_acc))
--	    f = f + 100 --arguably this shouldn't do anything to training
--	    print(f)
--	    f = f + 100 * math.abs(diff_acc)
--	    print(f)
--	    io.read()
--	    io.stderr:write()
--	    io.stderr:write(batch_num .. "," .. math.abs(diff_acc) .. "\n" )

---	    io.stderr:write(f .. "," .. acc_c1 .. "," .. acc_c2 .. "," .. math.abs(diff_acc) .. "\n" )
--	    io.read()
...
----------------------------------------------------------------------
function models.get_opt()
   return opt
end

function models.set_opt(new_opt)
   opt = new_opt
end
----------------------------------------------------------------------
--Train
