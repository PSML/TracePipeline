...
static unsigned char FwdPos[ALPHABETSIZE][W/2];
static unsigned char BwdPos[ALPHABETSIZE][W/2];
static char Excess[ALPHABETSIZE];
...
{ ulong bitW;
  ...
  while (res) {
    if (!minres || (res < minres)) {
      ...
      if ((i >= res) && (excess(i-res) == myexcess))
	minres = res;
    }
