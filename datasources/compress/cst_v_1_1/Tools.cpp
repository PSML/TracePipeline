...
alpha_t * Tools::GetRandomString(unsigned min, unsigned max, unsigned &alphabetSize)
{
  unsigned len = std::rand() % (max - min) + min;
  alphabetSize = std::rand() % 26 + 1;
  alpha_t* temp = new alpha_t[len + 2];
...
ulong * Tools::bp2bitstream(alpha_t* bp)
{
...
alpha_t *
Tools::GetFileContents(char *filename, trace_t *msize)
{
  trace_t maxSize = *msize;
  std::ifstream::pos_type posSize;
  std::ifstream file ((char *)filename, std::ios::in|std::ios::binary|std::ios::ate);
  if (file.is_open()) {
    posSize = file.tellg();
    trace_t size = posSize;
    if (maxSize != 0 && size > maxSize)
      size = maxSize;
    *msize = size / sizeof(alpha_t);
...
/*
  ulong Tools::ustrlen(alpha_t *text)
  {
  ulong i=0;
  ...
  char *Tools::uctoc(alpha_t *text, bool remove)
  {
  ...
  alpha_t *Tools::ctouc(char *text, bool remove)
  {
  ulong i, len = strlen(text);
   
  alpha_t *result = new alpha_t[len+1];
  ...
  void Tools::RemoveControlCharacters(alpha_t *data)
  {
