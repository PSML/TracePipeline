...
void lcss(char *filename, int rootonly)
{
  // A=aaabbbccc, B=ccbbbaa ==> lcss(A,B)=bbb
  //const char *A = "aaabbbccc", *B = "ccbbbaa";
  //const char *text = "aaabbbccc$ccbbbaa";
  //trace_t n = strlen((char *)text);;

  // One could read from file as follows:
  //char filename[] = "switch-6502-cc65.seq;
  trace_t n=0;
  alpha_t *text = Tools::GetFileContents(filename, &n);
  ...
  //sst->PrintSA();
  if (rootonly) {
    sst->PrintTree(sst->root(), 0, 0);
  //cout << "digraph G {" << endl;
  //cout << "  node[shape=point];" << endl;
  } else {
    sst->InitMyTree(sst->rightmost(sst->root()));
    sst->MyPrintTree(sst->root(), 0);
    sst->PrintAccuracy(1);
  }
  //cout << "}" << endl;
  //cout << endl;
...
{
  int printRootDepth1Children=0;

  if (argc < 2) {
    cout << "USAGE: " << argv[0] << " [seq file]" << endl;
    return -1;
  }
  if (argc > 2) printRootDepth1Children=1;
  lcss(argv[1],printRootDepth1Children);
  return 0;
}
